...
namespace Expenses.WebApi.Controllers
{
    ...
    public class ExpensesController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("deposit")]
        public IActionResult DepositFunds(DB.Expense expense)
        {
            var newExpense = _expensesServices.MakeDeposit(expense);
            return CreatedAtRoute("GetExpense", new { newExpense.Id }, newExpense);
        }

        [HttpPost("widthraw")]
        public IActionResult WidthrawFunds(DB.Expense expense)
        {
            var newExpense = _expensesServices.MakeWithdrawal(expense);
            return CreatedAtRoute("GetExpense", new { newExpense.Id }, newExpense);
        }

        [HttpPost("transfer")]
        public IActionResult TransferFunds(DB.Expense expense)
        {
            var newExpense = _expensesServices.MakeFundsTransfer(expense);
            return CreatedAtRoute("GetExpense", new { newExpense.Id }, newExpense);
        }
